                ...
                <div class="border-b border-slate-200 px-6 py-5 sm:px-8">
                    <div class="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
                        <div>
                            ...
                            <p class="mt-1 text-sm text-slate-500">A structured view of every report this enforcer has handled or is still processing.</p>
                        </div>
                        <div class="grid grid-cols-2 gap-3 sm:grid-cols-4">
                            <div class="rounded-2xl bg-slate-50 px-4 py-3 text-center ring-1 ring-slate-200">
                                ...
                                <p class="mt-1 text-xl font-black text-slate-900">{{ $totalAssigned }}</p>
                            </div>
                            <div class="rounded-2xl bg-amber-50 px-4 py-3 text-center ring-1 ring-amber-100">
                                <p class="text-[11px] uppercase tracking-wide text-amber-600">Active</p>
                                <p class="mt-1 text-xl font-black text-amber-900">{{ $activeCount }}</p>
                            </div>
                            <div class="rounded-2xl bg-blue-50 px-4 py-3 text-center ring-1 ring-blue-100">
                                <p class="text-[11px] uppercase tracking-wide text-blue-600">Review</p>
                                <p class="mt-1 text-xl font-black text-blue-900">{{ $reviewCount }}</p>
                            </div>
                            <div class="rounded-2xl bg-emerald-50 px-4 py-3 text-center ring-1 ring-emerald-100">
                                <p class="text-[11px] uppercase tracking-wide text-emerald-600">Resolved</p>
                                <p class="mt-1 text-xl font-black text-emerald-900">{{ $resolvedCount }}</p>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="overflow-x-auto">
                        <table class="min-w-full divide-y divide-slate-200 text-sm">
                            <thead class="bg-slate-50/80">
                                <tr class="text-left">
                                    <th class="px-6 py-4 font-semibold uppercase tracking-wide text-slate-500 sm:px-8">Report</th>
                                    <th class="px-6 py-4 font-semibold uppercase tracking-wide text-slate-500">Location</th>
                                    <th class="px-6 py-4 font-semibold uppercase tracking-wide text-slate-500">Timeline</th>
                                    <th class="px-6 py-4 font-semibold uppercase tracking-wide text-slate-500">Status</th>
                                    <th class="px-6 py-4 font-semibold uppercase tracking-wide text-slate-500 sm:px-8">Action</th>
                                </tr>
                            </thead>
                                    ...
                                    <tr class="align-top transition hover:bg-slate-50/80">
                                        <td class="px-6 py-5 sm:px-8">
                                            <div class="flex items-start gap-4">
                                                <div
                                                    class="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl bg-slate-900 text-xs font-black text-white">
                                                    #{{ $report->id }}
                                                </div>
                                                <div class="min-w-0">
                                        ...
                                        <td class="px-6 py-5 text-slate-600">
                                            <p class="font-medium text-slate-800">{{ $report->location }}</p>
                                            <p class="mt-1 text-xs text-slate-400">Reported {{ $report->created_at->diffForHumans() }}</p>
                                        </td>
                                        <td class="px-6 py-5 text-slate-600">
                                            <p class="font-medium text-slate-800">{{ $report->assigned_at?->format('M d, Y') ?? 'Not assigned' }}</p>
                                            <p class="mt-1 text-xs text-slate-400">{{ $report->assigned_at?->format('h:i A') ?? 'No assignment time recorded' }}</p>
                                        </td>
                                        <td class="px-6 py-5">
                                            ...
                                            <p class="mt-2 text-xs text-slate-500">
                                                @if($report->status === 'assigned')
                                                    In active field response.
                                                @elseif($report->status === 'for_verification')
                                                    Awaiting MITCOM validation.
                                                @elseif($report->status === 'resolved')
                                                    Case already completed.
                                                @else
                                                    Status recorded in the report workflow.
                                                @endif
                                            </p>
                                        </td>
                                        <td class="px-6 py-5 sm:px-8">
                                            <a href="{{ route('head-mitcom.reports.show', $report) }}"
                                                class="inline-flex items-center gap-2 rounded-xl border border-blue-200 bg-blue-50 px-4 py-2 text-xs font-semibold text-blue-700 transition hover:-translate-y-0.5 hover:border-blue-300 hover:bg-blue-100">
                                                View report
