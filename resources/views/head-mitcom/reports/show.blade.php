                        ...
                        @elseif($report->status === 'for_verification')
                            <div class="space-y-4" x-data="{ openProof: false }">
                                ...
                                <div class="rounded-2xl border border-cyan-200 bg-white p-4">
                                    ...
                                    <p class="mt-1 text-sm text-slate-500">Inspect the proof carefully before confirming or rejecting the closure.</p>
                                    ...
                                    {{-- Enforcer Remarks --}}
                                    @if($report->proof_remarks)
                                        <div class="mt-4 rounded-xl border border-slate-200 bg-slate-50 p-4">
                                            <p class="text-xs font-semibold uppercase tracking-wide text-slate-400">Enforcer Remarks</p>
                                            <p class="mt-2 text-sm leading-6 text-slate-700">{{ $report->proof_remarks }}</p>
                                        </div>
                                    @endif

                                    {{-- GPS Coordinates --}}
                                    @if($report->proof_latitude && $report->proof_longitude)
                                        <div class="mt-3 rounded-xl border border-emerald-100 bg-emerald-50 p-4">
                                            <p class="text-xs font-semibold uppercase tracking-wide text-emerald-600">GPS at Submission</p>
                                            <p class="mt-2 text-sm font-medium text-emerald-900">
                                                {{ number_format($report->proof_latitude, 5) }}, {{ number_format($report->proof_longitude, 5) }}
                                            </p>
                                            <a href="https://www.google.com/maps?q={{ $report->proof_latitude }},{{ $report->proof_longitude }}"
                                                target="_blank"
                                                class="mt-2 inline-flex items-center gap-1.5 text-xs font-semibold text-emerald-700 hover:text-emerald-900">
                                                View on Google Maps
                                                <svg class="h-3.5 w-3.5" viewBox="0 0 20 20" fill="currentColor">
                                    ...
                                    @else
                                        <div class="mt-3 rounded-xl border border-amber-100 bg-amber-50 p-3">
                                            <p class="text-xs text-amber-700">⚠ No GPS coordinates captured at submission.</p>
                                        </div>
                                    @endif

                                    <div class="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-2">
                                        <form action="{{ route('head-mitcom.reports.confirm-resolved', $report) }}" method="POST">
                                            @csrf
                                            <button type="submit"
                                        ...
                                        <form action="{{ route('head-mitcom.reports.reject-resolved', $report) }}" method="POST">
                                            @csrf
                                            <button type="submit"
                                                class="inline-flex w-full items-center justify-center rounded-xl bg-rose-600 px-4 py-3 text-sm font-semibold text-white transition hover:bg-rose-700">
                                                Reject proof
