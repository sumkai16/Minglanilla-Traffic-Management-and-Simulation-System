                ...
                <div class="px-6 py-5 border-b border-slate-200">
                    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                        ...
                        <div class="flex flex-wrap items-center gap-2" x-data="{ activeFilter: 'all' }">
                            <!-- TomTom Traffic Flow Toggle -->
                            <button id="traffic-toggle" onclick="toggleTrafficLayer()"
                                class="px-3 py-1.5 rounded-full text-xs font-semibold transition bg-white text-slate-700 ring-1 ring-slate-200 hover:bg-slate-50 flex items-center gap-1.5">
                                <span class="w-2 h-2 rounded-full inline-block"
                                    style="background: linear-gradient(to right, #22c55e, #ef4444);"></span>
                                Traffic Flow
                            </button>
                            <!-- Markers Toggle -->
                            <button id="markers-toggle" onclick="toggleMarkersLayer()"
                                class="px-3 py-1.5 rounded-full text-xs font-semibold transition bg-white text-slate-700 ring-1 ring-slate-200 hover:bg-slate-50 flex items-center gap-1.5">
                                <span class="w-2 h-2 rounded-full inline-block bg-blue-500"></span>
                                Hide Markers
                            </button>
        ...
        <script>
            ...
            function toggleMarkersLayer() {
                ...
                if (markersVisible) {
                    ...
                    btn.innerHTML = '<span class="w-2 h-2 rounded-full inline-block bg-slate-400"></span> Show Markers';
                    btn.classList.remove('bg-white', 'text-slate-700', 'ring-1', 'ring-slate-200');
                    btn.classList.add('bg-slate-700', 'text-white');
                } else {
                    markersLayer.addTo(map);
                    markersVisible = true;
                    btn.innerHTML = '<span class="w-2 h-2 rounded-full inline-block bg-blue-500"></span> Hide Markers';
