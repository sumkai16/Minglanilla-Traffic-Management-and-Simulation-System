            ...
            @foreach($statusColors as $status => $color)
                <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-5 hover:shadow-md transition cursor-default">
                    ...
                    <p class="text-3xl font-bold text-{{ $color }}-600 mt-1">
                        {{ \App\Models\Report::where('status', $status)->count() }}
                    </p>
                </div>
            @endforeach
        </div>
            ...
            <div class="px-6 py-5 border-b border-slate-200">
                <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between mb-4">
                    <div>
                        ...
                        <p class="text-xs text-slate-500 mt-1">Filter and manage incident reports from citizens</p>
                    </div>
                ...
                {{-- Filter Bar --}}
                <form method="GET" action="{{ route('admin.reports.index') }}" class="grid grid-cols-1 md:grid-cols-4 gap-3">
                    <div class="relative">
                    ...
                    <div>
                        <select name="issue_type"
                            class="w-full py-2 text-sm border border-slate-200 rounded-xl focus:ring-blue-500 focus:border-blue-500 text-slate-600 text-ellipsis overflow-hidden">
                    ...
                    <div class="flex gap-2">
                        ...
                        @if(request()->anyFilled(['search', 'status', 'issue_type']))
                            <a href="{{ route('admin.reports.index') }}" class="inline-flex items-center justify-center p-2 rounded-xl border border-slate-200 text-slate-400 hover:bg-slate-50 transition" title="Clear Filters">
                                <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
